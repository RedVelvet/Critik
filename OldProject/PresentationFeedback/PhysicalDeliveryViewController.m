...
- (void)viewDidLoad
{
    ...
    for (int j=0; j<[preCommentsDic6 count]; j++) {
        
        UIButton *button = (UIButton*)[self.view viewWithTag:[[[preCommentsDic6 allKeys] objectAtIndex:j] integerValue]];
        [button setImage:[UIImage imageNamed:@"CheckBoxChecked.jpeg"] forState:UIControlStateNormal];
        [button setSelected:YES];
    }
    
    for (int k=0; k<[quickGradesDic6 count]; k++) {
        
        NSString *index = [[quickGradesDic6 allKeys] objectAtIndex:k];
        NSLog(@"index  : %@",index);
        ...
        if([[quickGradesDic6 objectForKey:index] integerValue] == -1){
            ...
            //            [quickGradesDic6 setObject:@"-1" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
            
        }
        else if([[quickGradesDic6 objectForKey:index] integerValue] == 0){
            ...
            //           [quickGradesDic6 setObject:@"0" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
...
//    UITextField *textF = (UITextField*)[self.view viewWithTag:906];
//    
//    if([pointsArray count] > 2)
//        textF.text = [pointsArray objectAtIndex:textF.tag - 901];
//    
//    UITextView *textV = (UITextView*)[self.view viewWithTag:806];
//    
...
- (void) viewDidDisappear:(BOOL)animated
{
    ...
    UITextField *pts = (UITextField*)[self.view viewWithTag:906];
    
    [pointsArray setObject:pts.text atIndexedSubscript:pts.tag - 901];
    
    UITextView *writtenComments = (UITextView*)[self.view viewWithTag:806];
    [commentsArray setObject:writtenComments.text atIndexedSubscript:writtenComments.tag-801];
    
    
    for (int i=0; i<[quickGradesDic6 count]; i++) {
        UILabel *qGLabel = (UILabel*)[self.view viewWithTag:[[[quickGradesDic6 allKeys] objectAtIndex:i] integerValue]+100];
        quickGrdName = qGLabel.text;
        
        if([[quickGradesDic6 objectForKey:[[quickGradesDic6 allKeys] objectAtIndex:i]] integerValue] == -1)
            quickGradeVal = @"-";
        else if([[quickGradesDic6 objectForKey:[[quickGradesDic6 allKeys] objectAtIndex:i]] integerValue] == 0)
            quickGradeVal = @"Ok";
        else
            quickGradeVal = @"+";
        
        quickGrdsStr = [quickGrdsStr stringByAppendingFormat:@"%@ : %@\n", quickGrdName, quickGradeVal];
        [quickGradeStrArray setObject:quickGrdsStr atIndexedSubscript:5];
    }
    
    
    NSLog(@"Introduction DidDisappear");
    NSLog(@"hi : %@",quickGradesDic6);
...
- (IBAction)checkCommentButton:(id)sender {
    ...
    if(checkButton.selected)
    {
        ...
        [preCommentsDic6 setObject:((UILabel*)[self.view viewWithTag:[sender tag]+10000]).text forKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    else
    {
        [checkButton setImage:[UIImage imageNamed:@"CheckBox.jpeg"] forState:UIControlStateNormal];
        [preCommentsDic6 removeObjectForKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    NSLog(@"preCommentsDic6 : %@", preCommentsDic6);
}
