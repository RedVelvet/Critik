...
- (void)viewDidLoad
{
    ...
    NSLog(@"Reasoning and Evidence viewDidload");
    
    moduleName = @"Reasoning and Evidence";
    ...
    for (int j=0; j<[preCommentsDic3 count]; j++) {
        
        UIButton *button = (UIButton*)[self.view viewWithTag:[[[preCommentsDic3 allKeys] objectAtIndex:j] integerValue]];
        [button setImage:[UIImage imageNamed:@"CheckBoxChecked.jpeg"] forState:UIControlStateNormal];
        [button setSelected:YES];
    }
    
    for (int k=0; k<[quickGradesDic3 count]; k++) {
        
        NSString *index = [[quickGradesDic3 allKeys] objectAtIndex:k];
        NSLog(@"index  : %@",index);
        ...
        if([[quickGradesDic3 objectForKey:index] integerValue] == -1){
            ...
            //            [quickGradesDic3 setObject:@"-1" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
            
        }
        else if([[quickGradesDic3 objectForKey:index] integerValue] == 0){
            ...
            //           [quickGradesDic3 setObject:@"0" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
...
//    UITextField *textF = (UITextField*)[self.view viewWithTag:903];
//    textF.text = [pointsArray objectAtIndex:textF.tag - 901];
//    
//    UITextView *textV = (UITextView*)[self.view viewWithTag:803];
...
{
    NSLog(@"Reasoning and Evidence Diddisappear");
    ...
    UITextField *pts = (UITextField*)[self.view viewWithTag:903];
    
    [pointsArray setObject:pts.text atIndexedSubscript:pts.tag - 901];
    
    UITextView *writtenComments = (UITextView*)[self.view viewWithTag:803];
    [commentsArray setObject:writtenComments.text atIndexedSubscript:writtenComments.tag-801];
    
    
    for (int i=0; i<[quickGradesDic3 count]; i++) {
        UILabel *qGLabel = (UILabel*)[self.view viewWithTag:[[[quickGradesDic3 allKeys] objectAtIndex:i] integerValue]+100];
        quickGrdName = qGLabel.text;
        
        if([[quickGradesDic3 objectForKey:[[quickGradesDic3 allKeys] objectAtIndex:i]] integerValue] == -1)
            quickGradeVal = @"-";
        else if([[quickGradesDic3 objectForKey:[[quickGradesDic3 allKeys] objectAtIndex:i]] integerValue] == 0)
            quickGradeVal = @"Ok";
        else
            quickGradeVal = @"+";
        ...
        [quickGradeStrArray setObject:quickGrdsStr atIndexedSubscript:2];
    }
    
    
    NSLog(@"Introduction DidDisappear");
    NSLog(@"hi : %@",quickGradesDic3);
    
}

-(void) viewDidAppear:(BOOL)animated
{
    NSLog(@"Reasoning and Evidence Didappear");
}
...
- (IBAction)checkCommentButton:(id)sender {
    ...
    if(checkButton.selected)
    {
        ...
        [preCommentsDic3 setObject:((UILabel*)[self.view viewWithTag:[sender tag]+10000]).text forKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    else
    {
        [checkButton setImage:[UIImage imageNamed:@"CheckBox.jpeg"] forState:UIControlStateNormal];
        [preCommentsDic3 removeObjectForKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    NSLog(@"preCommentsDic3 : %@", preCommentsDic3);
}
