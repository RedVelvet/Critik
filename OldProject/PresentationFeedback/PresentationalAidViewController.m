...
- (void)viewDidLoad
{
    ...
    NSLog(@"Presentational Aid(s) viewDidload");
    
    moduleName = @"Presentational Aid(s)";
    ...
    for (int j=0; j<[preCommentsDic4 count]; j++) {
        
        UIButton *button = (UIButton*)[self.view viewWithTag:[[[preCommentsDic4 allKeys] objectAtIndex:j] integerValue]];
        [button setImage:[UIImage imageNamed:@"CheckBoxChecked.jpeg"] forState:UIControlStateNormal];
        [button setSelected:YES];
    }
    
    for (int k=0; k<[quickGradesDic4 count]; k++) {
        
        NSString *index = [[quickGradesDic4 allKeys] objectAtIndex:k];
        NSLog(@"index  : %@",index);
        ...
        if([[quickGradesDic4 objectForKey:index] integerValue] == -1){
            ...
            //            [quickGradesDic4 setObject:@"-1" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
            
        }
        else if([[quickGradesDic4 objectForKey:index] integerValue] == 0){
            ...
            //           [quickGradesDic4 setObject:@"0" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
...
//    UITextField *textF = (UITextField*)[self.view viewWithTag:904];
//    
//    if([pointsArray count] > 1)
//        textF.text = [pointsArray objectAtIndex:textF.tag - 901];
//    
//        UITextView *textV = (UITextView*)[self.view viewWithTag:804];
//        
...
{
    NSLog(@"Presentational Aid(s) Diddisappear");
    ...
    UITextField *pts = (UITextField*)[self.view viewWithTag:904];
    
    [pointsArray setObject:pts.text atIndexedSubscript:pts.tag - 901];
    
    UITextView *writtenComments = (UITextView*)[self.view viewWithTag:804];
    [commentsArray setObject:writtenComments.text atIndexedSubscript:writtenComments.tag-801];
    
    
    for (int i=0; i<[quickGradesDic4 count]; i++) {
        UILabel *qGLabel = (UILabel*)[self.view viewWithTag:[[[quickGradesDic4 allKeys] objectAtIndex:i] integerValue]+100];
        quickGrdName = qGLabel.text;
        
        if([[quickGradesDic4 objectForKey:[[quickGradesDic4 allKeys] objectAtIndex:i]] integerValue] == -1)
            quickGradeVal = @"-";
        else if([[quickGradesDic4 objectForKey:[[quickGradesDic4 allKeys] objectAtIndex:i]] integerValue] == 0)
            quickGradeVal = @"Ok";
        else
            quickGradeVal = @"+";
        ...
        [quickGradeStrArray setObject:quickGrdsStr atIndexedSubscript:3];
    }
    
    
    NSLog(@"Introduction DidDisappear");
    NSLog(@"hi : %@",quickGradesDic4);
    
}

-(void) viewDidAppear:(BOOL)animated
{
    NSLog(@"Presentational Aid(s) Didappear");
}
...
- (IBAction)checkCommentButton:(id)sender {
    ...
    if(checkButton.selected)
    {
        ...
        [preCommentsDic4 setObject:((UILabel*)[self.view viewWithTag:[sender tag]+10000]).text forKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    else
    {
        [checkButton setImage:[UIImage imageNamed:@"CheckBox.jpeg"] forState:UIControlStateNormal];
        [preCommentsDic4 removeObjectForKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    NSLog(@"preCommentsDic4 : %@", preCommentsDic4);
}
