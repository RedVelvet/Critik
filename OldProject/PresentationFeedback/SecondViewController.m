...
- (void)viewDidLoad
{
    ...
    for (int j=0; j<[preCommentsDic2 count]; j++) {
        
        UIButton *button = (UIButton*)[self.view viewWithTag:[[[preCommentsDic2 allKeys] objectAtIndex:j] integerValue]];
        [button setImage:[UIImage imageNamed:@"CheckBoxChecked.jpeg"] forState:UIControlStateNormal];
        [button setSelected:YES];
    }
    
    for (int k=0; k<[quickGradesDic2 count]; k++) {
        
        NSString *index = [[quickGradesDic2 allKeys] objectAtIndex:k];
        NSLog(@"index  : %@",index);
        ...
        if([[quickGradesDic2 objectForKey:index] integerValue] == -1){
            ...
            //            [quickGradesDic2 setObject:@"-1" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
            
        }
        else if([[quickGradesDic2 objectForKey:index] integerValue] == 0){
            ...
            //           [quickGradesDic2 setObject:@"0" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
    ...
    UITextField *textF = (UITextField*)[self.view viewWithTag:902];
    if([pointsArray count] > 1)
        textF.text = [pointsArray objectAtIndex:textF.tag - 901];
    else
        textF.text = @"";
    
    UITextView *textV = (UITextView*)[self.view viewWithTag:802];
...
- (void) viewDidDisappear:(BOOL)animated
{
    ...
    UITextField *pts = (UITextField*)[self.view viewWithTag:902];
    
    [pointsArray setObject:pts.text atIndexedSubscript:pts.tag - 901];
    
    UITextView *writtenComments = (UITextView*)[self.view viewWithTag:802];
    [commentsArray setObject:writtenComments.text atIndexedSubscript:writtenComments.tag-801];
    
    
    for (int i=0; i<[quickGradesDic2 count]; i++) {
        UILabel *qGLabel = (UILabel*)[self.view viewWithTag:[[[quickGradesDic2 allKeys] objectAtIndex:i] integerValue]+100];
        quickGrdName = qGLabel.text;
        
        if([[quickGradesDic2 objectForKey:[[quickGradesDic2 allKeys] objectAtIndex:i]] integerValue] == -1)
            quickGradeVal = @"-";
        else if([[quickGradesDic2 objectForKey:[[quickGradesDic2 allKeys] objectAtIndex:i]] integerValue] == 0)
            quickGradeVal = @"Ok";
        else
            quickGradeVal = @"+";
        ...
        [quickGradeStrArray setObject:quickGrdsStr atIndexedSubscript:1];
    }
    
    
    NSLog(@"hi : %@",quickGradesDic2);
    
}

-(void) viewDidAppear:(BOOL)animated
{
    NSLog(@"Secondview Didappear");
}
...
- (IBAction)checkCommentButton:(id)sender {
    ...
    if(checkButton.selected)
    {
        ...
        [preCommentsDic2 setObject:((UILabel*)[self.view viewWithTag:[sender tag]+10000]).text forKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    else
    {
        [checkButton setImage:[UIImage imageNamed:@"CheckBox.jpeg"] forState:UIControlStateNormal];
        [preCommentsDic2 removeObjectForKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    NSLog(@"preCommentsDic2 : %@", preCommentsDic2);
}
