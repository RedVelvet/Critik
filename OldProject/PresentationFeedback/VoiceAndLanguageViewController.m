...
- (void)viewDidLoad
{
    ...
    NSLog(@"Voice and Language viewDidload");
    
    moduleName = @"Voice and Language";
    ...
    for (int j=0; j<[preCommentsDic5 count]; j++) {
        
        UIButton *button = (UIButton*)[self.view viewWithTag:[[[preCommentsDic5 allKeys] objectAtIndex:j] integerValue]];
        [button setImage:[UIImage imageNamed:@"CheckBoxChecked.jpeg"] forState:UIControlStateNormal];
        [button setSelected:YES];
    }
    
    for (int k=0; k<[quickGradesDic5 count]; k++) {
        
        NSString *index = [[quickGradesDic5 allKeys] objectAtIndex:k];
        NSLog(@"index  : %@",index);
        ...
        if([[quickGradesDic5 objectForKey:index] integerValue] == -1){
            ...
            //            [quickGradesDic5 setObject:@"-1" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
            
        }
        else if([[quickGradesDic5 objectForKey:index] integerValue] == 0){
            ...
            //           [quickGradesDic5 setObject:@"0" forKey:[NSString stringWithFormat:@"%ld",(long)quickGradeButton.tag/1000]];
...
//    UITextField *textF = (UITextField*)[self.view viewWithTag:905];
//    
//    if([pointsArray count] > 2)
//        textF.text = [pointsArray objectAtIndex:textF.tag - 901];
//    
//        UITextView *textV = (UITextView*)[self.view viewWithTag:805];
//        
...
{
    NSLog(@"Voice and Language Diddisappear");
    ...
    UITextField *pts = (UITextField*)[self.view viewWithTag:905];
    
    [pointsArray setObject:pts.text atIndexedSubscript:pts.tag - 901];
    
    UITextView *writtenComments = (UITextView*)[self.view viewWithTag:805];
    [commentsArray setObject:writtenComments.text atIndexedSubscript:writtenComments.tag-801];
    
    
    for (int i=0; i<[quickGradesDic5 count]; i++) {
        UILabel *qGLabel = (UILabel*)[self.view viewWithTag:[[[quickGradesDic5 allKeys] objectAtIndex:i] integerValue]+100];
        quickGrdName = qGLabel.text;
        
        if([[quickGradesDic5 objectForKey:[[quickGradesDic5 allKeys] objectAtIndex:i]] integerValue] == -1)
            quickGradeVal = @"-";
        else if([[quickGradesDic5 objectForKey:[[quickGradesDic5 allKeys] objectAtIndex:i]] integerValue] == 0)
            quickGradeVal = @"Ok";
        else
            quickGradeVal = @"+";
        
        quickGrdsStr = [quickGrdsStr stringByAppendingFormat:@"%@ : %@\n", quickGrdName, quickGradeVal];
        [quickGradeStrArray setObject:quickGrdsStr atIndexedSubscript:4];
    }
    
    
    NSLog(@"Introduction DidDisappear");
    NSLog(@"hi : %@",quickGradesDic5);
    
}

-(void) viewDidAppear:(BOOL)animated
{
    NSLog(@"Voice and Language Didappear");
}
...
- (IBAction)checkCommentButton:(id)sender {
    ...
    if(checkButton.selected)
    {
        ...
        [preCommentsDic5 setObject:((UILabel*)[self.view viewWithTag:[sender tag]+10000]).text forKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    else
    {
        [checkButton setImage:[UIImage imageNamed:@"CheckBox.jpeg"] forState:UIControlStateNormal];
        [preCommentsDic5 removeObjectForKey:[NSString stringWithFormat:@"%ld",(long)[sender tag]]];
        
    }
    NSLog(@"preCommentsDic5 : %@", preCommentsDic5);
}
